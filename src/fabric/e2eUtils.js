        ...
        .then((enrollment) => {
            ...
            for (let key in ORGS[org]) {
                if (ORGS[org].hasOwnProperty(key)) {
                    if(key.indexOf('peer') === 0) {
                        let data = fs.readFileSync(commUtils.resolvePath(ORGS[org][key].tls_cacerts));
                        t.pass(commUtils.resolvePath(ORGS[org][key].tls_cacerts));
                        let peer = client.newPeer(
                            ORGS[org][key].requests,
                            {
                                'pem': Buffer.from(data).toString(),
...
function tlsEnroll(orgName) {
    return new Promise(function (resolve, reject) {
        ...
        let orgs = FabricCAServices.getConfigSetting('network');
        if (!orgs[orgName]) {
        ...
        let req = {
            enrollmentID: 'admin-' + orgName,
            enrollmentSecret: 'admin-' + orgName + 'pw',
            profile: 'tls'
        };
